...
export default function SubjectsPage() {
  ...
  async function getAllSubjectsFromAllPathways() {
    const pathwaysSnap = await getDocs(collection(db, 'pathways'));
    let allSubjects = [];
    for (const pathwayDoc of pathwaysSnap.docs) {
      const subjectsSnap = await getDocs(collection(db, 'pathways', pathwayDoc.id, 'subjects'));
      allSubjects = allSubjects.concat(subjectsSnap.docs.map(doc => ({
        id: doc.id,
        ...doc.data(),
        pathwayId: pathwayDoc.id
      })));
    }
    return allSubjects;
  }
  ...
  useEffect(() => {
    async function fetchSubjects() {
      ...
      try {
        const subjectsList = await getAllSubjectsFromAllPathways();
