...
export default function ProjectDetailPage() {
  ...
  // All calculations and hooks must be before any return
  ...
  const handleDeleteProject = async () => {
    ...
    try {
      ...
      router.push("/dashboard/activities?tab=projects");
    } catch (error) {
  ...
  useEffect(() => {
    ...
    if (progressMethod === 'manual') return;
    if (project.progress !== displayProgress || project.progressMethod !== progressMethod) {
      console.log('[SYNC] (HOOK) Syncing project progress to Firestore/activities:', {
        projectId,
        progress: displayProgress,
        progressMethod,
        prevProgress: project.progress,
        prevMethod: project.progressMethod
      });
      (async () => {
        await updateDoc(doc(db, "projects", projectId), { progress: displayProgress, progressMethod });
        await syncProjectToActivities(user.uid, projectId);
        console.log('[SYNC] (HOOK) Sync complete for project', projectId);
      })();
    }
  }, [displayProgress, progressMethod, project, user, db, projectId]);
    ...
    return (
      <div className="min-h-screen flex items-center justify-center">
        <div className="bg-red-900/20 p-8 rounded-lg text-center max-w-md">
          ...
          <Button onClick={() => router.push("/dashboard/activities?tab=projects")} className="mt-4">Back to Projects</Button>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className={`min-h-screen flex flex-col items-center py-10 px-4 ${themeStyles.textPrimary} ${themeStyles.cardBg}`}>
      <div className={`max-w-2xl w-full rounded-lg shadow-2xl p-8 ${themeStyles.cardBg} ${themeStyles.border} flex flex-col gap-6`}>
        ...
        <Button onClick={() => router.push("/dashboard/activities?tab=projects")} variant="outline" className={isDbzTheme ? 'text-black border-yellow-600 bg-yellow-50 hover:bg-yellow-100' : ''}>Back to Projects</Button>
      </div>
      ...
      <Dialog open={isAddItemDialogOpen} onOpenChange={setIsAddItemDialogOpen}>
        <DialogContent className={`${themeStyles.cardBg} ${themeStyles.border} ${isDbzTheme ? 'text-white' : ''}`}>
          ...
          />
          <textarea
            placeholder="Describe what you want to accomplish with this item (optional)"
            value={newItem.description}
            onChange={e => setNewItem({ ...newItem, description: e.target.value })}
            className={`mb-2 w-full rounded border border-slate-700 bg-slate-900 text-white px-3 py-2 focus:ring-2 focus:ring-blue-500 ${isDbzTheme ? 'text-white' : ''}`}
            rows={3}
          />
          {addItemType === 'task' && (
            <Input
